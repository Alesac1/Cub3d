...
int handle_resize(int x, int y, t_game *game)
{
    printf("Window resized to width: %d, height: %d\n", x, y);
	(void)game;
    // Update game state or perform any actions based on the new dimensions
    return (0);
}
...
int main(int argc, char const *argv[])
{
	...
	mlx_hook(game.mlx.window, 22, 0, &handle_resize, &game);
